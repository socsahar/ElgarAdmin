...
ALTER TABLE public.event_volunteer_assignments 
ADD COLUMN IF NOT EXISTS response_type TEXT DEFAULT 'assigned',
...
-- Add constraint for response_type values
ALTER TABLE public.event_volunteer_assignments 
DROP CONSTRAINT IF EXISTS event_volunteer_assignments_response_type_check;

ALTER TABLE public.event_volunteer_assignments 
ADD CONSTRAINT event_volunteer_assignments_response_type_check 
CHECK (response_type IN ('assigned', 'departure', 'arrived_at_scene', 'task_completed', 'cancelled'));
```);
    console.log('```');
